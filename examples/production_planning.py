...
try:
    ...
    if result["success"]:
        ...
        print("\nResource Usage:")
        labor_used = (product_a_qty * products["A"]["labor_hours"] + 
                     product_b_qty * products["B"]["labor_hours"])
        material_used = (product_a_qty * products["A"]["material_kg"] + 
                        product_b_qty * products["B"]["material_kg"])
        
        print(f"  Labor: {labor_used:.2f}/{resources['labor_hours']} hours " +
              f"({(labor_used/resources['labor_hours']*100):.1f}% utilized)")
        print(f"  Material: {material_used:.2f}/{resources['material_kg']} kg " +
